# PrediCT CAC Pipeline

## What is this?
PrediCT is an automated machine learning pipeline for computing the Coronary Artery Calcium (CAC) Agatston score from raw cardiac CT volumes. 
...
│   └── a3-coverage/  # Continuous probability Coverage Approach 3
└── data/             # Root output for inference jobs
```

## How to Run a Patient
Place a patient's DICOM folder into `data/uploads/<patient_id>`, then run the orchestrator:
```bash
python -m src.run --study <patient_id> --model a1-roi
```
Results, including `pred.nii.gz`, `ct.nii.gz`, `lesions.csv`, `run.json`, and PNG `slices/` will be generated in `data/out/<patient_id>/a1-roi`.
