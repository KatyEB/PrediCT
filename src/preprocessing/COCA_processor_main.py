...
# CONFIG
# ---------------------------------------------------------------------------
TARGET_SPACING = (0.37, 0.37, 3.0)   # mm — isotropic resampling target
USE_TOTAL_SEGMENTATOR = False        # Set False to skip heart masking
...
class COCAProcessor:
    ...
    def discover_series(self) -> list[Path]:
        ...
        if not self.dicom_root.exists():
            raise FileNotFoundError(
                f"DICOM root does not exist: {self.dicom_root}\n"
                f"Check the project_root argument passed to COCAProcessor."
            )
    ...
    def process_all(self):
        ...
        for s_dir in tqdm(series_dirs, desc="Processing Scans — TEST RUN"):
...
if __name__ == "__main__":
    processor = COCAProcessor(r"C:\SOHAM\coca_raw\cocacoronarycalciumandchestcts-2")
