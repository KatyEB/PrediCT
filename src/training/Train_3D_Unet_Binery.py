r"""
train_unet.py — PrediCT CAC Segmentation — 3D U-Net Baseline (binary)
=====================================================================
Trains a 3D U-Net on COCA coronary-calcium segmentation using the
{id}_seg.nii.gz binary masks.

Loss: Tversky (beta=0.7) — penalizes missed calcium (false negatives)
harder than false positives, suited to the extreme foreground imbalance.
Mixed precision: bfloat16 (fp32 range -> no overflow, no grad scaler).
Data: disk-cached pipeline for speed.

NOTE: changing the HU window invalidates the disk cache. Delete it before
the first run with new windowing:
  Remove-Item -Recurse -Force C:\SOHAM\cache\train, C:\SOHAM\cache\val

USAGE (PowerShell):
  & c:\SOHAM\vmenv\Scripts\python.exe c:/SOHAM/Train_3D_Unet.py `
    --data_root "C:\SOHAM\coca_raw\cocacoronarycalciumandchestcts-2\data_canonical\images" `
    ...
    --out_dir   "C:\SOHAM\runs\approach1_binary"

Outputs (in --out_dir):
  best_model.pth        best mean-Dice checkpoint
  train_log.csv         per-epoch metrics (mean + median Dice)
...
# ── Config (L4 24GB) ──────────────────────────────────────────────
HU_MIN, HU_MAX  = 0, 1200         # CAC window: calcium is >=130 HU, dense to 1000+
PATCH_SIZE      = (96, 96, 32)    # z smaller — only ~57 slices
POS_NEG_RATIO   = (1, 1)          # half of sampled patches contain calcium
SAMPLES_PER_VOL = 8               # patches drawn per volume per step
BATCH_SIZE      = 4               # volumes per batch
NUM_WORKERS     = 4               # bump to 8 if stable; drop to 0 if Windows errors
SW_OVERLAP      = 0.5             # sliding-window overlap at inference

CACHE_DIR_TRAIN = r"C:\SOHAM\cache\train"
CACHE_DIR_VAL   = r"C:\SOHAM\cache\val"


# ── Data list ─────────────────────────────────────────────────────
def build_data_list(csv_path, data_root, label_suffix):
    """Read a split file, return [{image, label}, ...] for scans on disk."""
...
def train_transforms():
    return Compose([
        ...
        # Foreground-biased sampling — the fix for extreme calcium imbalance
...
def val_transforms():
    # No augmentation, full volume -> honest Dice.
    return Compose([
        LoadImaged(keys=["image", "label"]),
        EnsureChannelFirstd(keys=["image", "label"]),
        Orientationd(keys=["image", "label"], axcodes="RAS"),
...
def build_model(device):
    return UNet(
        spatial_dims=3,
        in_channels=1,
        out_channels=1,          # binary: 1 logit channel + sigmoid
...
def main():
    ...
    ap.add_argument("--label_suffix", default="_seg")   # _seg_upsampled etc. for later approaches
    ap.add_argument("--epochs",       type=int,   default=200)
    ap.add_argument("--lr",           type=float, default=1e-3)   # lower peak — steadier on fine structure
    ...
    # Tversky: alpha penalizes false positives, beta penalizes false negatives.
    # beta > alpha biases toward recall — catch every calcium speck.
    ...
    # reduction="none" keeps per-scan Dice so we can report mean AND median
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            # bfloat16: fp32 dynamic range, no overflow -> no gradient scaler
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                outputs = model(imgs)
                loss = loss_fn(outputs, labels)

            # Skip a pathological batch rather than poison the epoch average
        ...
        if epoch % args.val_interval == 0:
            ...
            # per-scan Dice; nan = empty-GT (CAC-negative) scan, excluded
